...
inputFormBtn.on("click", function (){
    ...
    if(inputFormValue.length > 0){
        ...
        $('#workers-data-list').append("<div class='visi' style='display:flex; justify-content:space-between;'><span>You entered nothing!!</span><span id='btn-clear'><button  class='btn-clear' >Clear</button></span></div><br>");      
}
});

$('button.btn-clear').on("click", function () { 
    $('.visi').css("display", "none");
});
